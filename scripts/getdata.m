...
%% Connect to FTP host
host = 'ftp.data.pgc.umn.edu'; % ftp hostname
basefolder = '/aerial/usgs/tma/photos/'; % base folder for all photos data
s = ftp(host); % open connection
cd(s,basefolder); % go to the base folder which corresponds to our local target

folder = extractAfter(T.high_url(T.tma_num==tma_num(2)),basefolder);
